...
The notebook 'Analysis_file.ipynb' contains the generation of the first figures for paper [2] from the data created.
...
## Fig. 3,4 plot
The program 'direct_controlled_Lagrangian.ipynb; needs to be used to create the convergence and performance study, with the choices
- $\alpha=\gamma=0.5$
- $\alpha=0.5, \gamma=1$
- $\alpha=1, \gamma=0$

And the N values [50,100,150,200,250,300,350,400,450,500,550,600]
...
After running the code for each choice, the file 'plotting_data_pendulum.ipynb' generates the fig.3,4

## Fig. 6 plot
Fig. 6 considers the two-pendulum on a cart example and its data may be generated by using
'direct_controlled_Lagrangian_multi_pendulum.ipynb'.

Plot can be created using 'plotting_data_double_pendulum.ipynb'

## Fig. 7 plot
Fig. 7 illustrates the ability to solve rigid body problems in the form of rotating a rigd body using Euler-angles. Data can be created using the jupyter file 'direct_controlled_Lagrangian_rigid_body.ipynb'
